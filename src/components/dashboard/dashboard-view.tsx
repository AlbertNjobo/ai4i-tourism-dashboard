...
export default function DashboardView({ initialData, months, types, provinces }: Props) {
  const {
    filters,
    setMonths: ctxSetMonths,
    setTypes: ctxSetTypes,
    setProvinces: ctxSetProvinces,
    setMonth,
    resetFilters,
  } = useFilterContext()

  // Register available filter options with the context on mount
  useEffect(() => {
    ctxSetMonths(months)
    ctxSetTypes(types)
    ctxSetProvinces(provinces)
  }, [months, types, provinces, ctxSetMonths, ctxSetTypes, ctxSetProvinces])
  ...
  const {
    ...
    insights,
    resultCount,
  } = useTourismData(initialData, months, filters)
  ...
  return (
    <div className='flex flex-col gap-4'>
      ...
      <p className='text-muted-foreground text-sm'>
        Zimbabwe tourism performance across 8 destinations, Jan – Jun 2026. Use the filters in the sidebar to narrow by month, type, or province.
      </p>
